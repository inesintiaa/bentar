...
@section('content')
    <div class="d-flex flex-column gap-1">
        <div class="card">
            ...
            @endif
            <div class="card-body">
                <form class="mx-4" action="{{ route('admin.konser.update', $konser->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label for="name" class="form-label">Nama</label>
                        <input required type="text" class="form-control" id="name" name="name"
                            value="{{ $konser->name }}" data-custom-message="Tolong isi nama anda di sini.">
                    </div>
                    <div class="mb-3">
                        <label for="location" class="form-label">Lokasi</label>
                        <input required type="text" class="form-control" id="location" name="location"
                            value="{{ $konser->location }}" data-custom-message="Tolong isi lokasi konser di sini.">
                    </div>
                    <div class="mb-3 w-25">
                        <label for="date">Tanggal Konser</label>
                        <input required id="date" name="date" class="form-control" type="date"
                            value="{{ $konser->date }}" data-custom-message="Tolong isi tanggal konser di sini." />
                    </div>
                    <div class="mb-3 w-25">
                        ...
                        <div class="input-group date d-flex gap-2" id="timePicker">
                            <input name="start_time" id="start_time" type="text" class="form-control timePicker" required
                                value="{{ $konser->start_time }}" data-custom-message="Tolong isi jam konser dimulai di sini.">
